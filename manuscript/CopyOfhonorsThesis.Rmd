...
##Plots Interaction Effect between all 3 variables
read_file('../experiments/DF_pictures_1/jatos_results_20220216174226.txt') %>%
  # ... split it into lines ...
  str_split('\n') %>% first() %>%
  # ... filter empty rows ...
...
The main effect of cue instruction condition was, `r print_anova$full_result$encoding_instruction`. Mean accuracy in the remember condition was `r mean(subset(subject_means, encoding_instruction == "R")$mean_correct)`, and mean accuracy in the forget condition was `r mean(subset(subject_means, encoding_instruction == "F")$mean_correct)`. 

The main effect of test condition was, `r print_anova$full_result$test_condition`. Mean accuracy in the novel test condition was `r mean(subset(subject_means, test_condition == "novel")$mean_correct)`, and mean accuracy in the exemplar test condition was `r mean(subset(subject_means, test_condition == "exemplar")$mean_correct)`. 

The main effect of stimulus duration was, `r print_anova$full_result$encoding_stimulus_time`. Mean accuracy in the 500 ms condition was `r mean(subset(subject_means, encoding_stimulus_time == 500)$mean_correct)`, mean accuracy in the 1000 ms condition was `r mean(subset(subject_means, encoding_stimulus_time == 1000)$mean_correct)`, and mean accuracy in the 2000 ms condition was `r `r mean(subset(subject_means, encoding_stimulus_time == 2000)$mean_correct)``. 
