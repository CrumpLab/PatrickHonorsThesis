...
```{r}
load("../vignettes/data/E1/E1_data_write_up.RData")
```
...
The main effect of encoding time was, `r Accuracy$apa_print$full_result$encoding_stimulus_time`. Proportion correct was `r round(Accuracy[Accuracy$means$encoding_stimulus_time  == "500",]$mean_correct, digits=3)` (`r round(Accuracy[Accuracy$means$encoding_stimulus_time == "500",]$sem_correct,digits=3)`), `r round(Accuracy[Accuracy$means$encoding_stimulus_time == "1000",]$mean_correct, digits=3)` (`r round(Accuracy[Accuracy$means$encoding_stimulus_time == "1000",]$sem_correct,digits=3)`), and `r round(Accuracy[Accuracy$means$encoding_stimulus_time == "2000",]$mean_correct, digits=3)` (`r round(Accuracy[Accuracy$means$encoding_stimulus_time == "2000",]$sem_correct,digits=3)`) for the 500ms, 1000ms, and 2000ms intervals, respectively.

The main effect of test lure was significant, `r Accuracy$apa_print$test_condition`. Proportion correct was higher for novel lures (`r round(Accuracy[Accuracy$means$test_condition == "novel",]$grp_mean_correct, digits=3)`, SEM = `r round(Accuracy[Accuracy$means$test_condition == "novel",]$sem, digits=3)`), compared to exemplar lures (`r round(Accuracy[Accuracy$means$test_condition == "exemplar",]$grp_mean_correct, digits=3)`, SEM = `r round(Accuracy[Accuracy$means$test_condition == "exemplar",]$sem, digits=3)`).

The main effect of encoding instruction was, `r Accuracy$apa_print$full_result$encoding_instruction`. Proportion correct was similar for forget cued (`r round(Accuracy[Accuracy$means$encoding_instruction == "F",]$grp_mean_correct, digits=3)`, SEM = `r round(Accuracy[Accuracy$means$encoding_instruction == "F",]$sem, digits=3)`) and remember cued (`r round(Accuracy[Accuracy$means$encoding_instruction == "R",]$grp_mean_correct, digits=3)`, SEM = `r round(Accuracy[Accuracy$means$encoding_instruction == "R",]$sem, digits=3)`) items

The interaction between encoding instruction and encoding time was significant, `r Accuracy$apa_print$full_result$encoding_stimulus_time_test_condition`. To further interpret this interaction, paired sample t-tests were used to assess the directed forgetting effect at each encoding time duration. The directed forgetting effect is interpreted as a difference between proportion correct for remember minus forget items. At 500 ms, the directed forgetting effect was not detected, `r IT500_report$full_result`. At 1000ms, the directed forgetting effect was reversed, `r IT1000_report$full_result`. And, at 2000 ms, the directed forgetting effect was again not detected, `r IT2000_report$full_result`. The remaining interactions were not significant. 
