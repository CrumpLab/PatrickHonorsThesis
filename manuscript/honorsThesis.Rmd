...
CUE_Test = t.test(R_CUE, F_CUE, var.equal = TRUE)
...
Results of the analysis of variance indicates the lack of a significant effect of stimulus presentation duration. Participants had a higher average recognition accuracy proportion when exposed to  stimulus durations of 500 milliseconds  (M = `r EncDur_ME$mean_correct[1]`; SD = `r sd((filter(get_means, encoding_time == "500"))$mean_correct)`) and 1000 milliseconds  (M = `r EncDur_ME$mean_correct[2]`; SD = `r sd((filter(get_means, encoding_time == "1000"))$mean_correct)`), than when exposed to a stimulus duration of 2000 milliseconds  (M = `r EncDur_ME$mean_correct[3]`; SD = `r sd((filter(get_means, encoding_time == "2000"))$mean_correct)`),  F(1,10) = 0.074, p = 0.791.  
